...
export default function LightningTrackerModal({
  isOpen,
  onClose,
  city = 'Mumbai, MH',
  lat = 19.0760,
  ...
  // Derive genuine dynamic thermodynamic convective indicators from weather telemetry
  const convectiveMetrics = useMemo(() => {
    const isThunder = /thunder|storm|squall|lightning/i.test(condition);
    const isRainy = /rain|drizzle|shower|flood/i.test(condition) || rainChance > 45;
    ...
    const coordSeed = Math.abs(Math.sin(safeLat * 12.9898 + safeLng * 78.233)) * 1000;
    const baseDistance =
      riskLevel === 'severe'
        ? 2.1 + (coordSeed % 6.5)
        : riskLevel === 'moderate'
    ...
    const strikesCount30Min =
      riskLevel === 'severe'
        ? Math.round(18 + (coordSeed % 32))
        : riskLevel === 'moderate'
        ? Math.round(4 + (coordSeed % 8))
        : 0;

    const windAngleDeg = Math.round((coordSeed * 37) % 360);
    const windSpeedKmh = Math.round(12 + (coordSeed % 22));
  ...
  }, [safeLat, safeLng, condition, rainChance, temp, humidity]);

  // Generate dynamic real-time strike events around the coordinates
  const strikeEvents = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const angle = (i * (360 / Math.max(count, 1)) + (safeLat * 10) + i * 17) % 360;
      const rad = (angle * Math.PI) / 180;
      const dist = convectiveMetrics.nearestStrikeKm + (i * 2.8) + (i % 3);

      const strikeLat = safeLat + (dist / 111) * Math.cos(rad);
      const strikeLng = safeLng + (dist / (111 * Math.cos((safeLat * Math.PI) / 180))) * Math.sin(rad);

      const dirLabel = directions[Math.floor(((angle + 22.5) % 360) / 45)];
      const isCG = i % 3 !== 0;

      events.push({
        id: `strk-${i}-${Math.round(safeLat * 100)}`,
    ...
    return events;
  }, [safeLat, safeLng, convectiveMetrics]);
  ...
  }, [isOpen, safeLat, safeLng]);

  // Web Audio API Pulse Beep for warnings when unmuted
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Radar Concentric Range Rings (10km, 25km, 40km, 50km)
      ctx.strokeStyle = 'rgba(56, 189, 248, 0.25)';
      ctx.lineWidth = 1;

      [0.2, 0.5, 0.8, 1.0].forEach((scale, idx) => {
  ...
  const riskColors = {
    safe: {
      ...
      desc: `Atmospheric convective potential over ${city} is calm (CAPE: ${cape} J/kg). Zero dangerous thunderstorm cells detected within a 30 km radius. Safe for field responders and rescue teams.`,
    },
    moderate: {
    ...
    severe: {
      ...
      desc: `Active lightning and high electrostatic charge detected within ${nearestStrikeKm} km of ${city}. Storm cell is moving ${stormDir} at ${stormSpeedKmh} km/h (Lifted Index: ${liftedIndex}°C). Suspend open-field operations and seek immediate shelter!`,
    },
  }[riskLevel];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-4 bg-black/85 backdrop-blur-xl animate-fade-in">
      <div className="relative w-full max-w-2xl rounded-3xl bg-zinc-950/95 border border-yellow-400/40 shadow-2xl shadow-yellow-400/10 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-yellow-400/[0.04]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white font-mono flex items-center gap-2">
                <span>IITM / IMD DAMINI Lightning & Convective Analyzer</span>
              </h2>
              <div className="flex items-center gap-2 text-xs text-yellow-400 font-mono">
                <MapPin className="w-3.5 h-3.5 text-yellow-400" />
                <span>{city} ({safeLat.toFixed(2)}°N, {safeLng.toFixed(2)}°E)</span>
                <span>•</span>
                <span className="text-zinc-400">Live at {lastUpdated}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={isAlarmMuted ? 'Unmute Proximity Alarm' : 'Mute Proximity Alarm'}
        ...
        {/* Tab Navigation */}
        <div className="flex items-center gap-2 px-6 pt-3 pb-2 border-b border-zinc-800 bg-zinc-900/40">
          <button
        ...
        {/* Content Body */}
        <div className="p-6 space-y-4 font-mono text-xs text-zinc-200 overflow-y-auto scrollbar-thin scrollbar-thumb-yellow-400/30">
          {/* Main Status Card */}
          ...
          {activeTab === 'radar' && (
            <div className="space-y-4">
              <div className="flex flex-col items-center justify-center p-3 rounded-2xl bg-zinc-950 border border-zinc-800 relative overflow-hidden">
                <canvas
                  ref={canvasRef}
                  width={340}
                  height={320}
                ...
                />
                <div className="w-full flex items-center justify-between mt-2 px-2 text-[10px] text-zinc-400 font-mono">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
                <div className="p-3 rounded-2xl bg-zinc-900/70 border border-zinc-800 text-center space-y-1">
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider block">CAPE Index</span>
                ...
                <div className="p-3 rounded-2xl bg-zinc-900/70 border border-zinc-800 text-center space-y-1">
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider block">Lifted Index</span>
                  ...
                  <span className="text-[9px] text-zinc-400">{liftedIndex < 0 ? 'Severe Instability' : 'Stable Stratum'}</span>
                </div>

                <div className="p-3 rounded-2xl bg-zinc-900/70 border border-zinc-800 text-center space-y-1">
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider block">Nearest Activity</span>
                  <span className="text-base font-extrabold text-white block">{nearestStrikeKm} km</span>
                  <span className="text-[9px] text-zinc-400">{riskLevel === 'safe' ? 'Outside 30km' : 'Proximity Alert'}</span>
                </div>

                <div className="p-3 rounded-2xl bg-zinc-900/70 border border-zinc-800 text-center space-y-1">
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider block">Storm Track</span>
                  <span className="text-base font-extrabold text-cyan-400 block">{stormDir} • {stormSpeedKmh}km/h</span>
                  <span className="text-[9px] text-zinc-400">Surface vector</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'feed' && (
            <div className="space-y-2">
              ...
              {strikeEvents.length === 0 ? (
                <div className="p-8 rounded-2xl bg-zinc-900/60 border border-zinc-800 text-center text-zinc-400 space-y-2">
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                  {strikeEvents.map((st, idx) => (
                    <div
                      key={st.id}
                      className="p-3 rounded-xl bg-zinc-900/80 border border-zinc-800 flex items-center justify-between hover:border-yellow-400/40 transition"
                    >
                      <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] text-zinc-400">{Math.round(st.secondsAgo / 60)}m ago</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-3">
              <div className="p-4 rounded-2xl bg-zinc-900/80 border border-zinc-800 space-y-2.5">
                ...
                <ul className="space-y-2 text-xs text-zinc-300 list-disc list-inside leading-relaxed">
                  ...
                  <li>
                    <strong>If Thunder Roars, Go Indoors:</strong> Never remain in open agricultural fields, boats, or under solitary tall trees.
                  </li>
                  <li>
                    <strong>Avoid Metallic Conductors:</strong> Stay away from barbed wire fences, power cables, and open metal structures.
                  </li>
                  <li>
                    <strong>Safe Shelter:</strong> A pucca concrete building or fully enclosed vehicle is safe. Avoid tin sheds or temporary awnings.
                  </li>
                  <li>
                    <strong>30-Minute Post-Storm Rule:</strong> Wait at least 30 minutes after hearing the last thunderclap before resuming rescue work.
                  </li>
                </ul>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-cyan-950/20 border border-cyan-500/30 flex items-center gap-3">
                ...
                <p className="text-[11px] text-cyan-300 leading-relaxed">
                  Convective model integrated with Damini surface telemetry and atmospheric sounding index estimation.
                </p>
              </div>
            </div>
          )}
