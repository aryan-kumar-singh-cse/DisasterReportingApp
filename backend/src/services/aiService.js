/**
 * TwoTruths - AI Evidence Rule Engine & Dissonance Scoring Service
 */
...
/**
 * Evaluates detected image labels against user's claimed disaster type and severity
 */
function evaluateEvidence({ disasterType, userSeverity, labels = [] }) {
  ...
  if (userSevIdx !== -1 && aiSevIdx !== -1) {
    ...
    if (sevDelta >= 2 && dissonanceScore < 0.3) {
      dissonanceScore = Math.min(0.42, dissonanceScore + 0.2); // Elevate to partial dissonance
